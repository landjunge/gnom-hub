"""Cronjob Agent — plant und verwaltet zeitgesteuerte Aufgaben."""
...
MCP, NAME, POLL = "http://127.0.0.1:3100/sse", "CronjobAG", 10
SYS = "Format: @cron @Name [Aufgabe] [Zeit]. Nutze Tools. Antworte im Chat EXTREM KURZ in max 1 Zeile (z.B. '✅ Cronjob für @Name um 14:00 geplant.'). Keine Romane."
async def run():
    async with sse_client(MCP) as (r, w):
        async with ClientSession(r, w) as s:
            ...
            await s.call_tool("register_agent", {"name": NAME, "port": 0, "desc": "Plant Aufgaben"}); await s.call_tool("set_agent_status", {"a": NAME, "s": "online"})
            ...
            while True:
                ...
                new = [m for m in chat if m.get("id") not in seen and "@cron" in m.get("content","").lower()]
