...
def handle_clear(q=""):
    ...
    from gnom_hub.db.legacy_db import get_active_project; p = get_active_project()
    if q == "@projekt":
    ...
    if q.startswith("chat"):
        ...
        if len(parts) > 1:
            ...
            from gnom_hub.db.legacy_db import clear_project_chat_by_sender; clear_project_chat_by_sender(p, target_agent)
