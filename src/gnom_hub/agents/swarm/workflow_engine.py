...
def _record_wf_result(workflow_id: str, task_chain: list, overall_result: str,
    ...
    """Schreibt Workflow-Ergebnis in CoordinationDB (job_history + worker_stats).

    Die echte Signatur von ``CoordinationDB.record_job`` ist:
        record_job(worker, task_summary, result, duration_s, context_id=None, notes="")

    Frühere Versionen haben hier fälschlicherweise kwargs wie ``workflow_id``,
    ``task_chain``, ``overall_result`` etc. übergeben — das führte zu einem
    stillen ``TypeError``, der nur als ``logger.warning`` geloggt wurde. Folge:
    abgeschlossene/abgebrochene Workflows wurden nie in CoordinationDB
    gespeichert und waren damit auch nicht im Frontend sichtbar.

    Diese Funktion mappt die Workflow-Daten nun korrekt auf die echte
    Signatur und schreibt zusätzlich den (optionalen) failed_at_task in
    die ``notes``.
    """
    try:
        ...
        # Worker = "workflow" (es gibt keinen einzelnen Worker — Workflow ist
        # der Worker). task_summary = task_chain kompakt. result = overall_result.
        # duration_s = time.time() - start. notes = failed_at_task info.
        task_summary = " → ".join(capabilities) if capabilities else "(empty)"
        notes = ""
        if failed_at_task:
            notes = f"failed_at={failed_at_task}"
