...
function buildChatHintsHTML() {
  return `
    <div style="font-size:0.60rem; color:var(--text-dim); margin-top:8px; display:flex; gap:16px; align-items:center; padding: 2px 4px;">
      ...
      <span><strong>@@</strong> System-level command (e.g. @@project, @@status)</span>
    </div>
...
function cleanNormalChatMessage(safe) {
  ...
  safe = safe.replace(/\[BROWSER:\s*([^\]\n]+)\]/gi, function(match, action) {
    // HTML-escape to avoid XSS via agent-generated code
    const esc = String(action).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
    return `<div class="action-summary-badge browser-badge">🌐 <strong>Browser-Skript:</strong> <code>${esc.trim()}</code></div>`;
  });
  // Replace BROWSER output (post-processed by action_browser.py)
  safe = safe.replace(/\[Browser-Ausgabe:\s*([\s\S]*?)\]/g, function(match, body) {
    const esc = String(body).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
    return `<details class="action-summary-badge browser-badge"><summary>🌐 <strong>Browser-Ausgabe</strong></summary><pre style="white-space:pre-wrap; max-height:300px; overflow:auto; font-size:0.75rem; margin:4px 0 0 0;">${esc.trim()}</pre></details>`;
  });
  safe = safe.replace(/\[Browser-Fehler:\s*([^\]]+)\]/g, function(match, body) {
    const esc = String(body).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
    return `<div class="action-summary-badge browser-badge" style="border-color:#c33;">🌐 <strong>Browser-Fehler:</strong> <code>${esc.trim()}</code></div>`;
  });
  safe = safe.replace(/\[Browser-Timeout nach (\d+)s\.\s*([\s\S]*?)\]/g, function(match, secs, body) {
    const esc = String(body).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
    return `<div class="action-summary-badge browser-badge" style="border-color:#c93;">🌐 <strong>Browser-Timeout (${secs}s):</strong> <code>${esc.trim()}</code></div>`;
  });
  safe = safe.replace(/\[Browser: Sicherheitsüberprüfung fehlgeschlagen[^\]]*\]/g, function(match) {
    return `<div class="action-summary-badge browser-badge" style="border-color:#c33;">🛡️ <strong>Browser blockiert (Gatekeeper)</strong></div>`;
  });
  ...
  if (slides && Array.isArray(slides) && slides.length > 0) {
    setTimeout(() => {
      ...
      if (window.showboxPresentations && window.showboxPresentations[targetIdx]) {
        ...
        if (buttons) window.showboxPresentations[targetIdx].buttons = buttons;
      }
