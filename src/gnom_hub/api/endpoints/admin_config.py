...
def set_role(agent_id: str, role: str):
    ...
    result = implant(a["name"], c) if c else None
    return {"agent": a["name"], "role": role, "stored_in_db": result is not None}
...
# Sandbox-/Docker-Endpoints wurden am 2026-06-15 entfernt.
# Begründung: User wollte keine Docker- oder macOS-sandbox-exec-Pfade mehr.
# Befehle laufen direkt im Workspace-cwd.
# Gatekeeper (`verify_write` für Files) bleibt unverändert aktiv.
