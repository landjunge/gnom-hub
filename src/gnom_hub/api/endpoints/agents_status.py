...
def create_agent(a: AgentEntry, _=Depends(verify_admin)):
    repo = SQLiteAgentRepository()
...
def delete_agent(a_id: str, _=Depends(verify_admin)):
...
def update_agent_sliders(a_id: str, data: SliderUpdatePayload, _=Depends(verify_admin)):
    repo = SQLiteAgentRepository()
    agent = repo.get_by_id(a_id)
    if not agent:
        raise HTTPException(404, "Agent not found")
...
def delete_agent_blockade(a_id: str, blockade_id: int, _=Depends(verify_admin)):
    repo = SQLiteAgentRepository()
    agent = repo.get_by_id(a_id)
    if not agent:
        raise HTTPException(404, "Agent not found")
...
def toggle_agent_tool(a_id: str, data: ToolToggle, _=Depends(verify_admin)):
    repo = SQLiteAgentRepository()
    agent = repo.get_by_id(a_id)
    if not agent:
        raise HTTPException(404, "Agent not found")
