...
from .smart_crawl import _dom, _load, _save, rotate_user_agent, smart_request
...
def crawl_data(url, timeout=20):
    time.sleep(random.uniform(1.0, 2.5))  # noqa: S311 — Anti-Bot-Jitter, kein Crypto
    try:
        ...
        for letter in re.findall(r'<(?:ul|ol).*?</(?:ul|ol)>', html, re.DOTALL)[:5]: result += re.sub(r'\s+', ' ', re.sub(r'<[^>]+>', '', re.sub(r'<li[^>]*>', '• ', letter))).strip() + "\n"
