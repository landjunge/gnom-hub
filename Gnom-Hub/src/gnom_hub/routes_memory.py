...
def add_memory(e: MemoryEntry):
    ...
    n = {"id": str(uuid.uuid4()), **e.dict()}
    save_db("memory", get_db("memory") + [n])
    return n
@router.get("/api/memory/search")
def search_memory(q: str):
    return sorted([e for e in get_db("memory") if q.lower() in str(e.get("content", "")).lower()], key=lambda x: x.get("timestamp", ""), reverse=True)
@router.get("/api/agents/{a_id}/memory")
def get_agent_memory(a_id: str):
    return sorted([m for m in get_db("memory") if m.get("agent_id") == a_id], key=lambda x: x.get("timestamp", ""), reverse=True)
...
def delete_memory(m_id: str):
    save_db("memory", [m for m in get_db("memory") if m.get("id") != m_id])
