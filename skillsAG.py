"""Skills Agent — baut und verwaltet Agent-Skills."""
...
MCP, NAME, POLL = "http://127.0.0.1:3100/sse", "SkillsAG", 10
SYS = "Format: @skill @Name [Fähigkeit] [Bedingung]. Nutze Tools. Antworte im Chat EXTREM KURZ in max 1 Zeile (z.B. '✅ Skill für @Name erstellt.'). Keine Romane."
async def run():
    async with sse_client(MCP) as (r, w):
        async with ClientSession(r, w) as s:
            ...
            await s.call_tool("register_agent", {"name": NAME, "port": 0, "desc": "Baut Skills"}); await s.call_tool("set_agent_status", {"a": NAME, "s": "online"})
            ...
            while True:
                ...
                new = [m for m in chat if m.get("id") not in seen and "@skill" in m.get("content","").lower()]
